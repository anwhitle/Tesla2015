...
public class VisionTracking extends Command {
	//our attempt at vision tracking
	//Note that we copied this initially from another team and modified it for our purposes
	//Don't spend too much time on this since it is a bit advanced and very math oriented (I don't want you to be intimidated by it)
	...
	//for grey
	//NIVision.Range TOTE_HUE_RANGE = new NIVision.Range(0, 255);	//Default hue range for grey tote
	//NIVision.Range TOTE_SAT_RANGE = new NIVision.Range(0, 100);	//Default saturation range for grey tote
	//NIVision.Range TOTE_VAL_RANGE = new NIVision.Range(0, 50);	//Default value range for grey tote
    ...
    protected void execute() {
    	...
    	while(true){
			...
			//NIVision.imaqReadFile(frame, "/home/lvuser/image20.jpg");
			...
			CameraServer.getInstance().setImage(frame);  //originally binaryFrame
			...
			if(numParticles > 0)
			{
				...
				computeAlignment(binaryFrame, particles.elementAt(0));
			} else {
	...
	double computeDistance (Image image, ParticleReport report, boolean isLong) {
		...
		//SmartDashboard.putNumber("Width", size.width);
		return  targetWidth/(normalizedWidth*12*Math.tan(VIEW_ANGLE*Math.PI/(180*2)));
	}
	
	void computeAlignment(Image image, ParticleReport report){
		NIVision.GetImageSizeResult size;
		size = NIVision.imaqGetImageSize(image);
		
		if(report.BoundingRectLeft > 0){
			SmartDashboard.putBoolean("Move Right", true);
			SmartDashboard.putBoolean("Move Left", false);
		} else if(report.BoundingRectRight < size.width) {
		...
		SmartDashboard.putNumber("Percent Area to Image Area", report.PercentAreaToImageArea);
		SmartDashboard.putNumber("right side", report.BoundingRectRight);
		SmartDashboard.putNumber("left side", report.BoundingRectLeft);
